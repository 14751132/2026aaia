//week09-4 好玩的程式設計 (用陣列 for 迴圈， 畫很多氣球)
//修改week09-3 好玩的程式設計 (氣球會變大)
PImage img;// 宣告變數(圖)
void setup(){// 設定的函式(做1次)
  size(600,500);// 設定視窗大小
  img=loadImage("balloon.png");// 讀入圖片
}//要手動把[圖檔]拉近程式裡
float [] x = new float[200];//java陣列 x 座標
float [] y = new float[200];//java陣列 y 座標
float [] s = new float[200];//java陣列 s 氣球的大小
int N = 0 ;// 現在有幾個氣球
float s2 = 0.1; //一開始mouse按下去氣球的大小
void draw() {
  background(#FFFFF2);
